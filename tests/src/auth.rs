...
-----END PRIVATE KEY-----"
    ...
    let result = decrypt_password(key_pem.clone(), encrypted_data);
    assert!(result.is_ok());
    let decrypted_password = result.expect("ok");
    assert_eq!(decrypted_password, "xYsA0daSCDAEsmMDA0MDA");
    ...
    let result = decrypt_password(key_pem.clone(), encrypted_data);
    assert!(result.is_ok());
    assert_ne!(result.expect("ok"), "xYsA0daSCDAEsmMDA0MDA")
...
-----END PRIVATE KEY-----"
    ...
    let result = decrypt_password("INVALID_PRIVATE_KEY".to_string(), encrypted_data);
    assert!(result.is_err());
