...
fn checkout_basic() {
    ...
    assert_eq!(336, size_of::<stream::StreamMetrics>());
...
fn check_layout_rx_buffer() {
    assert_eq!((32, 104).select(), size_of::<rt::TxBuffer>());
}
#[ignore]
#[test]
...
fn check_stream_guard() {
    assert_eq!((152, 224).select(), size_of::<StreamGuard>());
}
#[ignore]
#[test]
fn check_stream() {
    assert_eq!((72, 152).select(), size_of::<Stream>());
}
#[ignore]
#[test]
fn check_handler() {
    assert_eq!((304, 456).select(), size_of::<Handler<'static>>());
    assert_eq!(
        (416, 568).select(),
