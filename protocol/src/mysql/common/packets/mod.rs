...
use crate::mysql::common::error::DriverError;
...
pub struct HandshakePacket<'a> {
    ...
    scramble_1: [u8; 8],
    __filler: Skip<1>,
    // lower 2 bytes，Capabilities flag的低2bytes
    capabilities_1: Const<CapabilityFlags, LeU32LowerHalf>,
    // character_set，server 默认charset
    default_collation: RawInt<u8>,
    // server status flag
    status_flags: Const<StatusFlags, LeU16>,
    // upper 2 bytes，Capabilities flag的高2bytes
    capabilities_2: Const<CapabilityFlags, LeU32UpperHalf>,
    // 合并的auth_plugin_data长度，即需要CLIENT_PLUGIN_AUTH
    ...
    proptest::proptest! {
        ...
        fn com_binlog_dump_roundtrip(
            ...
            let cmd = ComBinlogDump::new(server_id)
                ...
                .with_pos(pos)
                .with_flags(crate::mysql::common::packets::BinlogDumpFlags::from_bits_truncate(flags));
