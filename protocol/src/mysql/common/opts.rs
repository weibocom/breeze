...
use crate::mysql::common::constants::CapabilityFlags;
...
#[cfg(test)]
mod test {
    use crate::mysql::common::proto::codec::Compression;
    ...
    fn should_read_hashmap_into_opts() {
        use crate::mysql::common::opts::OptsBuilder;
        macro_rules!  map(
            { $($key:expr => $value:expr), + }=> {
                {
        ...
        assert_eq!(parsed_opts.opts.get_compress(), Some(Compression::best()));
        assert_eq!(
    ...
    fn should_have_url_err() {
        use crate::mysql::common::error::UrlError;
        use crate::mysql::common::opts::OptsBuilder;
        macro_rules!  map(
            { $($key:expr => $value:expr), + }=> {
                {
